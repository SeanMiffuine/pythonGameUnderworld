...
class MyGame(arcade.Window):
    ...
    def setup(self):
        ...
        for i in range(15):
            enemy = Enemy("enemy.png", 0.1)
            enemySpawn = False
            while not enemySpawn:

                # Randomly position
                enemy.center_x = random.randrange(AREA_WIDTH)
                enemy.center_y = random.randrange(AREA_HEIGHT)
    ...
    def draw_KIA(self):
        ...
        output = f"You were slain by a spirit"
        arcade.draw_text(output,
                         self.view_left + 350, self.view_bottom + 400,
    ...
    def on_draw(self):
        ...
        if self.current_state == INSTRUCTIONS_PAGE_0:
            ...
            self.draw_game()
            self.draw_game_over()
        elif self.current_state == GAME_KILLED:
            self.draw_game()
            self.draw_KIA()
    ...
    def update(self, delta_time):
        ...
        if self.current_state == GAME_RUNNING:
            ...
            for enemy in self.enemy_list:
                ...
                # Generate a list of all sprites that collided with the player.
            wound_list = arcade.check_for_collision_with_list(self.player_sprite, self.enemy_list)

            # Loop through each colliding sprite, remove it, and add to the score.
            for enemy in wound_list:
                self.current_state = GAME_KILLED
